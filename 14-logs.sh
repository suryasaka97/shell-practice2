...
echo -e "$Yellow Checking....whether you have mysql installed or not!$Reset" | tee -a $LOGS_FOLDER/$file_name
dnf list installed mysql &>> $LOGS_FOLDER/$file_name

if [ $? -ne 0 ]
then
    echo -e "$Red mysql is not installed...installing...$Reset"  | tee -a $LOGS_FOLDER/$file_name
    dnf install mysql -y  &>> $LOGS_FOLDER/$file_name
    validate $? mysql
else
    echo -e "$Yellow mysql is already installed$Reset" | tee -a $LOGS_FOLDER/$file_name
fi    

# Installing python3 #

echo -e  "$Yellow Checking....whether you have python installed or not!$Reset" | tee -a $LOGS_FOLDER/$file_name
dnf list installed python3 &>> $LOGS_FOLDER/$file_name

if [ $? -ne 0 ]
then
    echo -e "$Red python3 is not installed..installing$Reset" | tee -a $LOGS_FOLDER/$file_name
    dnf install python3 -y  &>> $LOGS_FOLDER/$file_name
    validate $? python3  &>> $LOGS_FOLDER/$file_name
else
    echo -e "$Yellow python3 is already installed$Reset" | tee -a $LOGS_FOLDER/$file_name
fi


# Installing Nginx #

echo -e "$Yellow Checking....whether you have nginx installed or not!$Reset" | tee -a $LOGS_FOLDER/$file_name
dnf list installed nginx  &>> $LOGS_FOLDER/$file_name

if [ $? -ne 0 ]
then
    echo -e "$Red nginx is not installed..installing$Reset"  | tee -a $LOGS_FOLDER/$file_name
    dnf install nginx -y  &>> $LOGS_FOLDER/$file_name
    validate $? nginx
else
    echo -e "$Yellow nginx is already installed$Reset"  | tee -a $LOGS_FOLDER/$file_name
fi
